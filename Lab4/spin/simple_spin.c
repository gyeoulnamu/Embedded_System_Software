...
static void simple_spin_read(struct str_st *buf) {
	//int ret;

	delay(5);
	spin_lock(&my_lock);
	copy_to_user(buf, kern_buf, sizeof(struct str_st));
...
static void simple_spin_write(struct str_st *buf) {
	//int ret;

	spin_lock(&my_lock);
	copy_from_user(kern_buf, buf, sizeof(struct str_st));
	spin_unlock(&my_lock);
...
static long simple_spin_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
	...
	switch(cmd) {
		case SIMPLE_SPIN_READ:
			simple_spin_read(user_buf);
			printk("simple_spin: ioctl read complete\n");
			//printk("simple_spin: ioctl read return %d\n", ret);
			break;
		case SIMPLE_SPIN_WRITE:
			simple_spin_write(user_buf);
			printk("simple_spin: ioctl write complete\n");
